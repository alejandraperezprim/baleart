<x-app-layout>
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('spaces.update', $space) }}" method="POST">
            ...
            <input type="text" name="name" value="{{ old('name', $space->name) }}" required class="border p-2 w-full rounded">

            <label class="block mt-4 mb-2">Total Score:</label>
            <input type="number" name="totalScore" value="{{ old('totalScore', $space->totalScore) }}" step="0.1" required class="border p-2 w-full rounded">

            <!-- Observaciones con CKEditor -->
            <label class="block mt-4 mb-2">Observaciones (CA):</label>
            <textarea name="observation_CA" id="editor_ca" class="border p-2 w-full rounded">{{ old('observation_CA', $space->observation_CA) }}</textarea>

            <label class="block mt-4 mb-2">Observaciones (ES):</label>
            <textarea name="observation_ES" id="editor_es" class="border p-2 w-full rounded">{{ old('observation_ES', $space->observation_ES) }}</textarea>

            <label class="block mt-4 mb-2">Observaciones (EN):</label>
            <textarea name="observation_EN" id="editor_en" class="border p-2 w-full rounded">{{ old('observation_EN', $space->observation_EN) }}</textarea>
